        ...
        <form role="form" action="<?= base_url() ?>CodeIgniterTraining/index.php/crudchannel/<?= isset($update) && $update ? 'update' : 'save' ?>" method="post">
            ...
            <div class="form-group">
                ...
                <label>Nama Saluran</label>
                    <input type="text" class="form-control" name="chanel_name" <?php echo ($update && $channel->num_rows() > 0) ? 'readonly' : ''; ?>>
            </div>
                    ...
                    <?php if ($update && $channel->num_rows() > 0) : ?>
                        ...
                        <div class="col-md-3">
                            <label class="radio-inline">
                                <input type="radio" name="channel_status" value="Aktif" <?= ($row->CHANNEL_STATUS == 'Aktif') ? 'checked' : 'disabled' ?> required> Ya
                            </label>
                        </div>
                        <div class="col-md-3">
                            <label class="radio-inline">
                                <input type="radio" name="channel_status" value="Tidak Aktif" <?= ($row->CHANNEL_STATUS == 'Tidak Aktif') ? 'checked' : 'disabled' ?> required> Tidak
                            </label>
                        </div>
                    <?php else : ?>
                        <div class="col-md-3">
